...
class MambaSplitConv1dScanCombinedFn(torch.autograd.Function):
    ...
    def forward(ctx, zxbcdt, conv1d_weight, conv1d_bias, dt_bias, A, D, chunk_size, initial_states=None, seq_idx=None, dt_limit=(0.0, float("inf")), return_final_states=False, activation="silu",
        ...
        xBC_conv = rearrange(causal_conv1d_cuda.causal_conv1d_fwd(xBC_t, conv1d_weight, conv1d_bias, seq_idx, None, None, activation in ["silu", "swish"]), "b d s -> b s d")
    ...
    def backward(ctx, dout, *args):
        ...
        # xBC_t = rearrange(xBC, "b s d -> b d s").contiguous()
        # dxBC_t = rearrange(dxBC, "b s d -> b d s").contiguous()
        # dxBC_given, dweight, dbias, *_ = causal_conv1d_cuda.causal_conv1d_bwd(
        #         xBC_t, conv1d_weight, conv1d_bias, dxBC_t, seq_idx, None, None, 
        #         rearrange(dxBC_given, "b s d -> b d s"), False, ctx.activation in ["silu", "swish"]
        # )
        # dxBC_given = rearrange(dxBC_given, "b d s -> b s d")
    
        x_t = rearrange(xBC, "b s d -> b d s").contiguous()
        dy_t = rearrange(dxBC, "b s d -> b d s").contiguous()
        grad_in_t = rearrange(dxBC_given, "b s d -> b d s").contiguous()
        
        dx_t, dweight, dbias, *_ = causal_conv1d_cuda.causal_conv1d_bwd(
                x_t, 
                conv1d_weight, 
                conv1d_bias, 
                dy_t, 
        ...
        dxBC_given = rearrange(dx_t, "b d s -> b s d")
